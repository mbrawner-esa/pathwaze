...
| **R-15** | **Workload heatmap on the Gantt** | S–M | Shade each month/quarter column by how much lands in it, so the intensity of upcoming work is visible before anyone reads a single date — the question "when are we about to be swamped" is currently only answerable by counting markers. Inputs already exist: the Gantt on `/health` (`PriorityGantt` in [PriorityBoard.tsx](src/components/health/PriorityBoard.tsx)) already has every open milestone with a target date positioned on a month axis, so this is a per-column aggregate plus a fill, not new data. ⚠️ Two design notes: weight by **critical-path and milestone count separately** — ten routine items is not the same load as two critical ones — and per the dataviz rules a magnitude shade is a **sequential single-hue ramp**, never the red/amber/green status palette already used for health, or the two encodings will be read as the same thing. |
