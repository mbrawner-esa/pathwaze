...
| 9 | **@-mention in task/notes editors** | ✅ Done | `mentionUsers` now wired into task description (create+edit), project new-task + note/event composers, and offtaker-pricing notes; mentions email the user + log a feed entry (create-time). **Note:** the Slack-style thread/comment composers (task comments, `ThreadsTab`, pricing thread messages) still use the separate `<@USERID>` token system — unifying those is a separate follow-up. |
